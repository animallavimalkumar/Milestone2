...
def home(request):
    ...
    return render(request, 'home.html', {'expenses': expenses, 'income': income})
...
def dashboard(request):
    total_income = Income.objects.filter(user=request.user).aggregate(total=Coalesce(Sum('amount'), 0))['total']
    total_expenses = Expense.objects.filter(user=request.user).aggregate(total=Coalesce(Sum('amount'), 0))['total']
    budgets = Budget.objects.filter(user=request.user)
    budget_summary = [
        {
            'category': budget.category.name,
            'limit': budget.amount_limit,
            'spent': Expense.objects.filter(user=request.user, category=budget.category)
                     .aggregate(spent=Coalesce(Sum('amount'), 0))['spent']
        }
        for budget in budgets
    ]
    emi_summary = EMI.objects.filter(user=request.user, end_date__gte=date.today()).order_by('next_payment_date')
